...
const App = () => {
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-100 font-sans selection:bg-emerald-500/30 pb-24 flex flex-col">
  ...
  return (
    <>
      ...
      <div className="flex flex-col">
        {/* Заменяем h1 на изображение */}
        <img 
          src="/text.png" // Или {logo}, если вы делали импорт
          alt="SKALYK Logo"
          className="h-16 w-auto object-contain mb-1" // h-8 (32px) обычно идеально для шапки
        />
      </div>
      ...
      <div className="p-6 space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-gradient-to-br from-zinc-900 to-zinc-900 border border-zinc-800 p-5 rounded-[2rem] relative overflow-hidden flex flex-col justify-between min-h-[160px]">
            ...
            <div className="relative z-10">
              ...
              {readiness === 'ready' ? (
                ...
                <>
                  <h2 className="text-3xl font-black text-amber-500 mb-1">ACTIVE REST</h2>
                </>
              )}
            </div>
            <div className="relative z-10 mt-4">
               <p className="text-2xl text-zinc-300 italic font-medium leading-tight mb-2">"{quote.text}"</p>
               <p className="text-1.5xl text-zinc-500 font-bold uppercase tracking-wider">— {quote.character}, <span className="text-emerald-500">{quote.source}</span></p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="bg-zinc-900 p-5 rounded-[2rem] border border-zinc-800 flex flex-col min-h-[140px]">
              {/* Заголовок: как был до этого (слева) */}
              <div className={`flex items-center gap-2 mb-2 ${trendPercent >= 0 ? 'text-emerald-500' : 'text-red-500'}`}>
              ...
              <div className="flex-grow flex items-center justify-center text-center">
                <span className={`text-4xl font-black leading-none ${trendPercent >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
          ...
          <div className="flex justify-between items-center mb-6">
            ...
            {(chartType === 'cumulative' || chartType === 'daily_routes') && (
               <div className="flex gap-2 text-[9px] font-bold uppercase hidden sm:flex">
                  <span className="flex items-center gap-1"><div className="w-2 h-2 bg-emerald-500 rounded-full"></div>Easy</span>
                  <span className="flex items-center gap-1"><div className="w-2 h-2 bg-blue-500 rounded-full"></div>Normal</span>
                  <span className="flex items-center gap-1"><div className="w-2 h-2 bg-red-500 rounded-full"></div>Hard</span>
               </div>
            )}
          </div>
          ...
          <div className="h-64 pb-4 flex items-end justify-between gap-1">
            {chartType === 'grade_dist' ? (
              gradeDistData.map((item) => {
                ...
                const colorClass = 
                  item.category === 'easy' ? 'bg-emerald-500' : 
                  item.category === 'normal' ? 'bg-blue-500' : 'bg-red-500';
                ...
                if (chartType === 'load') {
                  ...
                  chartBar = (
                    <div className="relative w-full max-w-[16px] flex items-end h-full">
                       ...
                       {day.sends > 0 && (
                          <div style={{ height: `${Math.max(5, sendsH)}%` }} className="absolute bottom-0 w-full bg-emerald-400 z-10 rounded-sm shadow-[0_0_8px_rgba(52,211,153,0.5)]" />
                       )}
                    </div>
                  );
                  ...
                  chartBar = (
                    <div className="relative w-full max-w-[16px] flex items-end h-full">
                      <div className="absolute bottom-0 w-full h-full bg-zinc-800 rounded-sm opacity-20" />
                      {day.attempts > 0 && (
                         <div style={{ height: `${Math.max(2, rate)}%` }} className={`absolute bottom-0 w-full rounded-sm ${rate > 0 ? 'bg-emerald-400 shadow-[0_0_8px_rgba(52,211,153,0.5)]' : 'bg-zinc-600'}`} />
                      )}
                    </div>
                  );
                  ...
                  tooltipContent = (
                    <div>
                      ...
                      <div className="flex gap-2 text-[8px]">
                        <span className="text-emerald-400">E:{day.cumEasy}</span>
                        <span className="text-blue-400">N:{day.cumNormal}</span>
                        <span className="text-red-400">H:{day.cumHard}</span>
                      </div>
                    </div>
                  );
                  chartBar = (
                    <div className="w-full max-w-[16px] flex flex-col justify-end relative h-full">
                       ...
                       <div className="relative w-full flex flex-col justify-end" style={{ height: `${totalH}%` }}>
                          <div className="w-full bg-red-500 rounded-t-sm" style={{ height: `${day.totalCum > 0 ? (day.cumHard / day.totalCum) * 100 : 0}%` }} />
                          <div className="w-full bg-blue-500" style={{ height: `${day.totalCum > 0 ? (day.cumNormal / day.totalCum) * 100 : 0}%` }} />
                          <div className="w-full bg-emerald-500 rounded-b-sm" style={{ height: `${day.totalCum > 0 ? (day.cumEasy / day.totalCum) * 100 : 0}%` }} />
                       </div>
                    </div>
                  );
                  ...
                  tooltipContent = (
                    <div>
                      ...
                      <div className="flex gap-2">
                        <span className="text-emerald-400">E:{day.dailyEasy}</span>
                        <span className="text-blue-400">N:{day.dailyNormal}</span>
                        <span className="text-red-400">H:{day.dailyHard}</span>
                      </div>
                    </div>
                  );
                  chartBar = (
                    <div className="w-full max-w-[16px] flex flex-col justify-end relative h-full">
                       ...
                       {day.dailyTotal > 0 && (
                          <div className="relative w-full flex flex-col justify-end" style={{ height: `${Math.max(2, totalH)}%` }}>
                              <div className="w-full bg-red-500 rounded-t-sm" style={{ height: `${day.dailyTotal > 0 ? (day.dailyHard / day.dailyTotal) * 100 : 0}%` }} />
                              <div className="w-full bg-blue-500" style={{ height: `${day.dailyTotal > 0 ? (day.dailyNormal / day.dailyTotal) * 100 : 0}%` }} />
                              <div className="w-full bg-emerald-500 rounded-b-sm" style={{ height: `${day.dailyTotal > 0 ? (day.dailyEasy / day.dailyTotal) * 100 : 0}%` }} />
                          </div>
                       )}
                    </div>
                  );
...
const HistoryView = ({ 
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      ...
      days.push(
        ...
        >
          ...
          {hasData && (
            <div className={`w-1.5 h-1.5 rounded-full mt-1 ${isSelected ? 'bg-black' : 'bg-emerald-500'}`} />
          )}
        </button>
      );
    }
      ...
      <div className="min-h-screen bg-zinc-950 p-6">
        ...
        <div className="space-y-6">
                   ...
                   <div className="flex items-center gap-3">
                      ...
                      <div>
                         ...
                         <div className="text-xs text-zinc-500">
                           <span className="text-emerald-500 font-bold">{attempt.projectGrade}</span> • {attempt.outcome === 'send' ? 'SENT' : `Fall: Move ${attempt.fallMove}`}
                         </div>
                      </div>
                   </div>
                   <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="bg-zinc-900/30 border border-zinc-800/50 rounded-2xl p-2 focus-within:ring-1 focus-within:ring-emerald-500/50 transition-all">
              <textarea 
                value={dayLog}
  ...
  return (
    <div className="min-h-screen bg-zinc-950 pb-20 p-6">
      ...
      <div className="mb-6">
        <h2 className="text-lg font-bold text-emerald-500 mb-4 uppercase tracking-widest text-center">
        ...
        <div className="space-y-6">
          <div>
            ...
            <input 
              ...
              className="w-full bg-zinc-900 border border-zinc-800 rounded-2xl p-4 text-white text-lg focus:outline-none focus:border-emerald-500"
            />
          </div>
          <div>
            ...
            <textarea 
              ...
              className="w-full bg-zinc-900 border border-zinc-800 rounded-2xl p-4 text-white text-sm focus:outline-none focus:border-emerald-500 min-h-[150px]"
            />
          </div>
          <button onClick={handleAdd} className="w-full py-5 bg-emerald-500 text-white rounded-3xl font-black text-xl mt-4">SAVE PROTOCOL</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="p-6 pt-12 bg-zinc-950 min-h-screen pb-24">
      <div className="flex justify-between items-end mb-6">
        ...
        <button onClick={() => setIsAdding(true)} className="text-emerald-500 font-bold text-sm bg-emerald-500/10 px-4 py-2 rounded-xl border border-emerald-500/20 hover:bg-emerald-500/20">+ ADD</button>
      </div>
                  ...
                  <div className="px-6 pb-6 pt-0">
                    ...
                    <div className="flex justify-end">
                      <button 
                        ...
                        className="flex items-center gap-2 text-red-500 bg-red-500/10 px-4 py-2 rounded-xl text-sm font-bold"
                      >
      ...
      {mode === 'stopwatch' ? (
          ...
          <div className="flex gap-4 w-full max-w-sm">
            <button 
              ...
              className={`flex-1 py-8 rounded-[2rem] flex items-center justify-center gap-2 text-xl font-black transition-all ${
                swRunning ? 'bg-zinc-800 text-white' : 'bg-emerald-500 text-zinc-950'
              }`}
            >
        ...
        <div className="flex flex-col items-center w-full">
          ...
          <div className="mb-8 text-center">
             ...
             <p className="text-emerald-500 font-bold uppercase tracking-widest flex items-center justify-center gap-2">
          ...
          <div className="flex gap-4 w-full max-w-sm mb-10">
            <button 
              ...
              className={`flex-1 py-8 rounded-[2rem] flex items-center justify-center gap-2 text-xl font-black transition-all ${
                timerRunning ? 'bg-zinc-800 text-white' : 'bg-emerald-500 text-zinc-950'
              }`}
            >
             ...
             <div className="flex gap-4 items-center justify-center">
                <div className="flex flex-col items-center">
                  <input 
                    ...
                    className="w-20 bg-zinc-950 border border-zinc-800 rounded-xl p-3 text-2xl font-bold text-center text-white" 
                  />
    ...
    <div className="min-h-screen bg-zinc-950 p-6 pb-24">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div 
           ...
           className="w-full h-48 rounded-2xl border-2 border-dashed border-zinc-800 flex flex-col items-center justify-center cursor-pointer hover:border-emerald-500/50 hover:bg-zinc-900 transition-all relative overflow-hidden group"
        >
           <input 
              type="file" 
              ref={fileInputRef} 
        ...
        <div>
          ...
          <input 
            ...
            className="w-full bg-zinc-900 border border-zinc-800 rounded-xl p-4 text-white focus:border-emerald-500 outline-none transition-colors"
        ...
        <div className="grid grid-cols-2 gap-4">
           <div>
             ...
             <select 
               ...
               className="w-full bg-zinc-900 border border-zinc-800 rounded-xl p-4 text-white focus:border-emerald-500 outline-none appearance-none"
             >
          ...
          <div className="flex flex-wrap gap-2 mb-3">
            {(formData.style || []).map(s => (
              <button
                ...
                className="px-3 py-1.5 rounded-full text-xs font-bold uppercase tracking-wider border bg-emerald-500 border-emerald-500 text-black flex items-center gap-1 hover:bg-red-500 hover:border-red-500 hover:text-white group transition-colors"
              >
          ...
          <div className="flex gap-2 mb-4">
             <div className="relative flex-1">
                ...
                <input 
                   ...
                   className="w-full bg-zinc-900 border border-zinc-800 rounded-xl py-3 pl-10 pr-4 text-white text-sm focus:border-emerald-500 outline-none"
        ...
        <button 
          type="submit"
          className="w-full bg-emerald-500 text-black font-bold uppercase tracking-wider p-4 rounded-xl mt-8 hover:bg-emerald-400 active:scale-[0.98] transition-all"
        >
          Save Project
        </button>
      </form>
    ...
    <div className="min-h-screen bg-zinc-950 pb-24 relative">
       ...
       <div className="h-96 bg-gradient-to-b from-zinc-800 to-zinc-950 relative group">
          ...
          <div className="absolute top-0 left-0 w-full p-6 flex justify-between items-start z-10 pt-8 bg-gradient-to-b from-black/80 to-transparent pb-12">
             ...
             <div className="flex gap-2">
                ...
                <button onClick={onDelete} className="p-2 bg-black/50 backdrop-blur rounded-full text-red-500 hover:bg-red-500/20"><Trash2 className="w-5 h-5" /></button>
          ...
          <div className="absolute bottom-0 left-0 w-full p-6 bg-gradient-to-t from-zinc-950 via-zinc-950/80 to-transparent pt-24">
             <div className="flex items-end justify-between">
                <div>
                   ...
                   <div className="flex items-center gap-3">
                      <span className="px-3 py-1 bg-emerald-500 text-black text-xs font-bold rounded-md">{project.grade}</span>
       ...
       <div className="p-6 space-y-8 -mt-4 relative z-10">
          ...
          <button 
             ...
             className="w-full bg-emerald-500 text-black font-black uppercase tracking-wider p-4 rounded-2xl shadow-[0_0_20px_rgba(16,185,129,0.3)] hover:shadow-[0_0_30px_rgba(16,185,129,0.5)] transition-all active:scale-[0.98]"
          >
             Log Attempt
          </button>
          ...
          <div className="grid grid-cols-3 gap-4">
             ...
             <div className="bg-zinc-900 p-4 rounded-2xl border border-zinc-800 text-center">
                ...
                <div className={`text-xl font-black uppercase ${project.status === 'sent' ? 'text-emerald-500' : 'text-zinc-300'}`}>
                   {project.status}
                </div>
             </div>
          </div>
          ...
          <div>
            ...
            <textarea 
               ...
               className="w-full bg-zinc-900/50 border border-zinc-800 rounded-2xl p-4 text-zinc-300 text-sm min-h-[100px] focus:outline-none focus:border-emerald-500/50"
             ...
             <div className="space-y-3">
                {[...project.attempts].reverse().map((attempt) => (
                   <div key={attempt.id} className="flex items-center justify-between p-4 bg-zinc-900 rounded-2xl border border-zinc-800">
                      <div className="flex items-center gap-3">
                         {attempt.outcome === 'send' ? <CheckCircle className="text-emerald-500 w-5 h-5" /> : <XCircle className="text-zinc-600 w-5 h-5" />}
                         <div>
             ...
             <div className="bg-zinc-900 w-full max-w-md rounded-3xl p-6 border border-zinc-800 space-y-6">
                ...
                <div className="flex gap-2">
                   <button 
                      ...
                      className={`flex-1 p-4 rounded-xl font-bold border-2 transition-all ${newAttempt.outcome === 'send' ? 'border-emerald-500 bg-emerald-500/20 text-emerald-500' : 'border-zinc-800 text-zinc-500'}`}
                   >
                      SEND
                   </button>
                   <button 
                      onClick={() => setNewAttempt({...newAttempt, outcome: 'fall'})}
                      className={`flex-1 p-4 rounded-xl font-bold border-2 transition-all ${newAttempt.outcome === 'fall' ? 'border-amber-500 bg-amber-500/20 text-amber-500' : 'border-zinc-800 text-zinc-500'}`}
                   >
                      FALL
                   </button>
                </div>
    ...
    <div className="fixed bottom-0 left-0 right-0 bg-zinc-950 border-t border-zinc-900 pb-safe pt-2 px-6 h-20 flex justify-around items-start z-40">
      {tabs.map(tab => {
        ...
        return (
          ...
          >
             <div className={`p-2 rounded-2xl transition-all duration-300 ${isActive ? 'bg-emerald-500 text-black translate-y-[-10px] shadow-lg shadow-emerald-500/20' : 'text-zinc-500 group-hover:text-zinc-300'}`}>
                <Icon className="w-6 h-6" />
             </div>
             <span className={`text-[10px] font-bold uppercase transition-all ${isActive ? 'text-emerald-500 translate-y-[-8px]' : 'text-zinc-600'}`}>
                {tab.label}
             </span>
          </button>
        );
